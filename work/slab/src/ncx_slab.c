...
void
ncx_slab_stat(ncx_slab_pool_t *pool, ncx_slab_stat_t *statt)
{
    ...
    ncx_memzero(statt, sizeof(ncx_slab_stat_t));
    ...
    page = pool->pages;
    statt->pages = (pool->end - pool->start) / ncx_pagesize;;

    for (i = 0; i < statt->pages; i++)
    {
        slab = page->slab;
        type = page->prev & NCX_SLAB_PAGE_MASK;
        ...
        switch (type) {
            ...
            case NCX_SLAB_SMALL:
                ...
                for (j = 0; j < map; j++) {
                    for (m = 1 ; m; m <<= 1) {
                        if ((bitmap[j] & m)) {
                            statt->used_size += obj_size;
                            statt->b_small   += obj_size;
                        }
            ...
            case NCX_SLAB_EXACT:
                ...
                if (slab == NCX_SLAB_BUSY) {
                    statt->used_size += sizeof(uintptr_t) * 8 * ncx_slab_exact_size;
                    statt->b_exact   += sizeof(uintptr_t) * 8 * ncx_slab_exact_size;
                }
                else {
                    for (m = 1; m; m <<= 1) {
                        if (slab & m) {
                            statt->used_size += ncx_slab_exact_size;
                            statt->b_exact    += ncx_slab_exact_size;
                        }
                    }
                }
            ...
            case NCX_SLAB_PAGE:
                ...
                if (page->prev == NCX_SLAB_PAGE) {		
                    ...
                    statt->used_size += slab * ncx_pagesize;
                    statt->b_page    += slab * ncx_pagesize;
                    statt->p_page    += slab;
            ...
            default:
                ...
                if (slab  > statt->max_free_pages) {
                    statt->max_free_pages = page->slab;
                }

                statt->free_page += slab;
    ...
    statt->used_pct = statt->used_size * 100 / statt->pool_size;

    info("pool_size : %zu bytes",	statt->pool_size);
    info("used_size : %zu bytes",	statt->used_size);
    info("used_pct  : %zu%%\n",		statt->used_pct);

    info("total page count : %zu",	statt->pages);
    info("free page count  : %zu\n",	statt->free_page);
        
    info("small slab use page : %zu,\tbytes : %zu",	statt->p_small, statt->b_small);	
    info("exact slab use page : %zu,\tbytes : %zu",	statt->p_exact, statt->b_exact);
    info("big   slab use page : %zu,\tbytes : %zu",	statt->p_big,   statt->b_big);	
    info("page slab use page  : %zu,\tbytes : %zu\n",	statt->p_page,  statt->b_page);				

    info("max free pages : %zu\n",		statt->max_free_pages);
}
